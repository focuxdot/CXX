...
# C叉叉是一个把你电脑上的 Codex / Claude Code 安全接到手机上的远程接管工具。
...
> [!NOTE]
> Windows / Linux 架构已预留，随后跟进。手机端是网页，iOS / 安卓 / 微信内置浏览器均可用。

1. **启动菜单栏程序**（见下方「[构建 macOS App](#-构建-macos-app)」，或等待发布版）。首次未开启远程，界面按系统语言显示中文或英文。
