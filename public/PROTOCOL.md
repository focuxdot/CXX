...
**能力协商（caps）**：client 在 auth 参数里带 `caps` 对象声明自身能力（走加密信道，中继剥不掉）：`seq:1` 信封序号（见 §2）、`img:1` 接收图片推送流（见 §3.2）。daemon 在成功应答里回执自身能力 `caps:{img:1}`（全权设备；观众连接不推图故不回执）。旧 daemon 忽略未知参数、不回执 caps，双方自然回落旧行为。
...
daemon 可同时服务多个 agent 后端（`codex` 与 `claude`，后者需本机装有 Claude Code CLI）。手机端首页左上角下拉切换 agent，每个会话相关请求带 `agent` 字段路由到对应后端（缺省 `codex`，向后兼容）。

```json
{"id":2,"method":"agents.list","params":{}}
// result: {"agents":[{"id":"codex","name":"Codex","healthy":true},
//                    {"id":"claude","name":"Claude Code","healthy":true}]}  // 仅注册成功的后端
```

- **带 `agent` 的方法**：`sessions.list`、`projects.list`、`session.watch`、`session.send`、`session.new`、`session.start`、`session.fork`、`turn.interrupt`、`goal.*`、`models.list`、`approval.respond`、`share.create`、`share.list`、`share.react`。缺省 `codex`；未注册的 agent 回落 `codex`。
...
// Codex 后端：基于当前会话调用 app-server thread/fork 并打开新分支。result: {"threadId":"...","cwd":"..."}
```
