...
**能力协商（caps）**：client 在 auth 参数里带 `caps` 对象声明自身能力（走加密信道，中继剥不掉）：`seq:1` 信封序号（见 §2）、`img:1` 接收图片推送流（见 §3.2）。daemon 在成功应答里回执自身能力 `caps:{img:1,rtc:1}`（全权设备；观众连接不推图故不回执；`rtc:1` 表示支持局域网直连升级，见 §3.9）。旧 daemon 忽略未知参数、不回执 caps，双方自然回落旧行为。
